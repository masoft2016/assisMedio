...
public class Contexto implements java.io.Serializable, ConID {
    @Id
    @TableGenerator(name = "ctx",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "CTX",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "ctx")
    @Basic(optional = false)
    @Column(name = "CTX_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "CTX_NOMBRE", length=80)
    private String nombre;
    @Basic(optional = false)
    @Column(name = "CTX_CALIDAD")
    private Integer calidad;
    @Basic(optional = true)
    @Column(name = "CTX_IDVIEJO")
