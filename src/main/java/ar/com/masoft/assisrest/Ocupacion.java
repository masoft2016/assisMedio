...
public class Ocupacion implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "ocu",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "OCU",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "ocu")
    @Basic(optional = false)
    @Column(name = "OCU_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "OCU_NOMBRE", length=80)
    private String nombre;
    @Column(name = "OCU_IDVIEJO")
