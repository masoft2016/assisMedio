...
public class Mantenimiento implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "mnt",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "MNT",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "mnt")
    @Basic(optional = false)
    @Column(name = "MNT_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "MNT_NOMBRE", length=80)
    private String nombre;
    @Basic(optional = false)
    @Column(name = "MNT_CALIDAD")
    private Integer calidad;
    @Column(name = "MNT_IDVIEJO")
