...
public class Ingresos implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "ing",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "ING",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "ing")
    @Basic(optional = false)
    @Column(name = "ING_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "ING_NOMBRE", length=80)
    private String nombre;
    @Column(name = "ING_IDVIEJO")
