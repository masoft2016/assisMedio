...
public class Profesion implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "pro",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "PRO",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "pro")
    @Basic(optional = false)
    @Column(name = "PRO_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "PRO_NOMBRE", length=80)
    private String nombre;
    @Column(name = "PRO_IDVIEJO")
