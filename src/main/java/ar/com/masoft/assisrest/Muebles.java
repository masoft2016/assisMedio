...
public class Muebles implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "mue",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "MUE",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "mue")
    @Basic(optional = false)
    @Column(name = "MUE_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "MUE_NOMBRE", length=80)
    private String nombre;
    @Basic(optional = false)
    @Column(name = "MUE_CALIDAD")
    private Integer calidad;
    @Column(name = "MUE_IDVIEJO")
