...
public class RilasciatoDa implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "rid",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "RID",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "rid")
    @Basic(optional = false)
    @Column(name = "RID_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "RID_NOMBRE", length=80)
    private String nombre;
    @Column(name = "RID_IDVIEJO")
