...
public class AssistenzaEstado implements java.io.Serializable, ConID{
    @Id
    @TableGenerator(name = "ase",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "ASE",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "ase")
    @Basic(optional = false)
    @Column(name = "ASE_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "ASE_NOMBRE", length=80)
    private String nombre;
    @Column(name = "ASE_IDVIEJO")
    @Basic(optional = true)
