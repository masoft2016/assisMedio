...
public class Electrodomestico implements java.io.Serializable, ConID {
    @Id
    @TableGenerator(name = "elc",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "ELC",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "elc")
    @Basic(optional = false)
    @Column(name = "ELC_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "ELC_NOMBRE", length=80)
    private String nombre;
    @Basic(optional = false)
    @Column(name = "ELC_CALIDAD")
    private Integer calidad;
    @Column(name = "ESC_IDVIEJO")
