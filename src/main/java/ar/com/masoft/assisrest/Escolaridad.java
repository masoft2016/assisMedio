...
public class Escolaridad implements java.io.Serializable, ConID {
    @Id
    @TableGenerator(name = "esc",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "ESC",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "esc")
    @Basic(optional = false)
    @Column(name = "ESC_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "ESC_NOMBRE", length=80)
