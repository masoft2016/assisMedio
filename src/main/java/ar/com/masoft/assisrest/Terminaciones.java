...
public class Terminaciones implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "ter",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "TER",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "ter")
    @Basic(optional = false)
    @Column(name = "TER_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "TER_NOMBRE", length=80)
    private String nombre;
    @Basic(optional = false)
    @Column(name = "TER_CALIDAD")
    private Integer calidad;
    @Column(name = "TER_IDVIEJO")
