...
public class Obrasocial implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "oss",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "OSS",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "oss")
    @Basic(optional = false)
    @Column(name = "OSS_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "OSS_NOMBRE", length=80)
    private String nombre;
    @Column(name = "OSS_IDVIEJO")
