...
public class Distrito implements java.io.Serializable, ConID {
    @Id
    @TableGenerator(name = "dis",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "DIS",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "dis")
    @Basic(optional = false)
    @Column(name = "DIS_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "DIS_NOMBRE", length=80)
    private String nombre;
    @Column(name = "DIS_IDVIEJO")
