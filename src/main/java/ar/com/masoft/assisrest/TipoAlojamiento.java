...
public class TipoAlojamiento implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "tal",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "TAL",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "tal")
    @Basic(optional = false)
    @Column(name = "TAL_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "TAL_NOMBRE", length=80)
    private String nombre;
    @Basic(optional = false)
    @Column(name = "TAL_CALIDAD")
    private Integer calidad;
    @Column(name = "TAL_IDVIEJO")
