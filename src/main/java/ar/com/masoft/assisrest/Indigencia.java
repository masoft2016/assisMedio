...
public class Indigencia implements java.io.Serializable,ConID {
    @Id
    @TableGenerator(name = "ind",
            table = "numerador",
            pkColumnName = "tipo",
            valueColumnName = "numero",
            pkColumnValue = "IND",
            allocationSize = 1, initialValue = 1)
    @GeneratedValue(strategy = GenerationType.TABLE, generator = "ind")
    @Basic(optional = false)
    @Column(name = "IND_ID")
    private Integer id;
    @Basic(optional = false)
    @Column(name = "IND_NOMBRE", length=80)
    private String nombre;
    @Basic(optional = false)
    @Column(name = "IND_NIVEL")
    private Integer nivel;
    @Column(name = "IND_IDVIEJO")
